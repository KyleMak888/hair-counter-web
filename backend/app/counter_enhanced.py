...
class EnhancedCounterConfig(CounterConfig):
    ...
    scale_factors: tuple[float, ...] = (0.8, 1.0, 1.2)
...
def count_dark_clusters_enhanced(
    image: np.ndarray,
    config: EnhancedCounterConfig | None = None
) -> CountResult:
    """Enhanced detection with multi-scale and adaptive thresholding."""
    ...
    for scale in scales:
        candidates = _detect_at_scale(processed_image, scale, config)
